...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    async function loadImpostazioni() {
      const { data } = await supabase.from('impostazioni').select('percentuale_gioco, num_slot').eq('id', 1).single()
      if (data) {
  ...
  function calcSchedule(base) {
    ...
    const quaterna = Math.floor(base * 0.19 / 2)
    const full     = Math.floor(base * 0.04)
