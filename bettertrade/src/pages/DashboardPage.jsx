...
// ── Manometro SVG ─────────────────────────────────────────────────────────────
// Scala: -100% → +500% mappata su arco da 225° a 315° (passando per 270° in alto)
// In SVG: 0°=destra, 90°=basso, 180°=sinistra, 270°=alto
// Manometro classico: parte da sx (225°) sale in alto (270°) e scende a dx (315°)
function Manometro({ pct }) {
  // Clamp tra -100 e 500
  const val   = Math.max(-100, Math.min(500, pct || 0))
  // Mappa val su angolo: -100 → 225°, 500 → 315° (senso orario, passando per 270°)
  // Range totale 600 punti percentuali su 270 gradi
  const angle = 225 + ((val + 100) / 600) * 270
  ...
  // Archi colorati — sweep=1 = senso orario
  function arcPath(startPct, endPct, radius) {
    const a1  = (225 + ((startPct + 100) / 600) * 270) * Math.PI / 180
    const a2  = (225 + ((endPct   + 100) / 600) * 270) * Math.PI / 180
    const x1s = cx + radius * Math.cos(a1), y1s = cy + radius * Math.sin(a1)
    const x2e = cx + radius * Math.cos(a2), y2e = cy + radius * Math.sin(a2)
    const large = Math.abs(endPct - startPct) / 600 * 270 > 180 ? 1 : 0
    return `M ${x1s} ${y1s} A ${radius} ${radius} 0 ${large} 1 ${x2e} ${y2e}`
  }

  // Tacche sui valori chiave
  function tick(pctVal, inner, outer) {
    const a = (225 + ((pctVal + 100) / 600) * 270) * Math.PI / 180
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center' }}>
      <svg width="320" height="200" viewBox="0 0 320 200">
        ...
        {/* Maschera per nascondere la parte inferiore */}
        <rect x="0" y="168" width="320" height="35" fill="#090909"/>
        ...
        <text x={cx} y={cy+36} textAnchor="middle"
          style={{fontSize:22, fontWeight:700, fill: '#e0d9d0', fontFamily:"'DM Mono',monospace"}}>
          {val > 0 ? '+' : ''}{val.toFixed(1)}%
        </text>
      </svg>
    </div>
