...
export default function ReportingPage() {
  ...
  async function salvaGiornata() {
    ...
    if (isSuperAdmin) {
      // SuperAdmin: somma i calcoli di tutti gli utenti (stessa logica di salvaPerTutti)
      const {data:freshUsers}=await supabase.from('users').select('*').neq('role','superadmin')
      const allUsers=freshUsers||[]

      // Calcola spinData aggregato: somma investito/incasso/saldo di ogni utente
      const spinDataAgg = [0,1,2,3].map(si => {
        const tiles = spins[si]||[]
        if (!tiles.some(t=>t.casa)) return null
        let totInv=0, totInc=0
        for (const u of allUsers) {
          const uBase=((u.bankroll||0)*(pct/100))/numSlot
          const uSched=calcSchedule(uBase)
          const rawTiles=tiles.map(t=>({...t,
            result:t.result===true||t.result===1?'win':
                   t.result===false||t.result===0?'loss':
                   (t.result||'').toString().toLowerCase()
          }))
          totInv+=calcInvestito(uSched)
          totInc+=calcIncasso(rawTiles,uSched)
        }
        const totInvR=parseFloat(totInv.toFixed(2))
        const totIncR=parseFloat(totInc.toFixed(2))
        return {investito:totInvR, incasso:totIncR, saldo:parseFloat((totIncR-totInvR).toFixed(2))}
      })

      const totInvestito=spinDataAgg.reduce((s,d)=>s+(d?.investito||0),0)
      const totIncasso=spinDataAgg.reduce((s,d)=>s+(d?.incasso||0),0)
      ...
      const {data:newG}=await supabase.from('giornate').insert([{
        ...
        spins:spinDataAgg, tot_investito:totInvestito, tot_incasso:totIncasso, tot_saldo:totSaldo,
      ...
      // Utente normale: calcola con il suo bankroll personale
