...
export default function ReportingPage() {
  ...
  async function eliminaGiornata(id) {
    // Trova la data della giornata selezionata
    const giornata = giornate.find(g => g.id === id)
    if (!giornata) return
    // Elimina per tutti gli utenti con la stessa data
    await supabase.from('giornate').delete().eq('data', giornata.data)
    setGiornate(prev => prev.filter(g => g.data !== giornata.data))
    setConfirmIdx(null)
  }
