...
export default function ReportingPage() {
  ...
  async function salvaGiornata() {
    ...
    // Calcola base: SuperAdmin usa totale aggregato, utenti normali usano proprio bankroll
    let myBase
    if (isSuperAdmin) {
      const {data:allUsers}=await supabase.from('users').select('bankroll').neq('role','superadmin')
      const totalBk=(allUsers||[]).reduce((s,u)=>s+(u.bankroll||0),0)
      myBase=(totalBk*(pct/100))/numSlot
    } else {
      ...
      myBase=(freshBankroll*(pct/100))/numSlot
    }
