  ...
  return (
    <div style={S.page}>
          ...
          return (
            <div key={u.id} style={{ ...S.row, borderBottom: i < visible.length-1 ? '1px solid #181818' : 'none' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
                ...
                <div style={{background:'#0a0a0a',border:'1px solid #1e1e1e',borderRadius:6,padding:'5px 12px',display:'flex',alignItems:'center',gap:4,minWidth:90}}>
                  <span style={{fontSize:11,color:'#555',fontFamily:"'DM Mono',monospace"}}>€</span>
                  <span style={{fontSize:13,color:'#c9a84c',fontFamily:"'DM Mono',monospace",fontWeight:500}}>
                    {(u.bankroll||0).toFixed(2)}
                  </span>
                </div>
                {isSuperAdmin && u.role !== 'superadmin' && (
                  <button style={{...S.delBtn, color:'#60a5fa', background:'rgba(59,130,246,0.08)', border:'1px solid rgba(59,130,246,0.18)'}}
                    onClick={()=>{ setChangePwUser(u); setNewPw(''); setPwOk('') }}>🔑</button>
                )}
                {isSuperAdmin && u.role !== 'superadmin' && (
      ...
      {isSuperAdmin && users.length > 0 && (
        <div style={{...S.row, borderTop:'1px solid #1e1e1e', marginTop:4}}>
          <span style={{fontSize:13,fontWeight:700,color:'#e0d9d0',fontFamily:"'Sora',sans-serif"}}>Totale aggregato</span>
          <div style={{display:'flex',alignItems:'center',gap:4}}>
            <span style={{fontSize:11,color:'#555',fontFamily:"'DM Mono',monospace"}}>€</span>
            <span style={{fontSize:16,fontWeight:700,color:'#c9a84c',fontFamily:"'DM Mono',monospace"}}>
