...
export default function BilancioPage() {
  ...
  useEffect(() => {
    async function init() { await fetchUsers(); setLoading(false) }
    init()
  }, [])
  ...
  function addWeekEntry() {
    ...
    saveStorico({ ...storico, [userId]: [...current, { label: newWeek.label, importo: parseFloat(newWeek.importo) }] })
    setNewWeek({ label:'', importo:'' }); setShowAddWeek(false)
  }
  function removeWeekEntry(userId, idx) {
    const current = storico[userId] || []
    saveStorico({ ...storico, [userId]: current.filter((_,i) => i !== idx) })
  }
