...
export default function BilancioPage() {
  ...
  useEffect(() => {
    async function init() {
      await fetchUsers()
      if (currentUser) {
        // Carica tutte le stagioni per il bilancio cumulativo
        const { data } = await supabase.from('giornate').select('*').eq('user_id', currentUser.id).order('stagione').order('week_number')
          ...
          {[...myStorico].reverse().map((entry) => (
            <div key={entry.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 16px', borderBottom:'1px solid #181818' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                ...
                {isSuperAdmin && (
                  <button onClick={() => removeWeekEntry(entry.id)}
