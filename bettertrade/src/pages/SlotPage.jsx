        ...
        {tiles.map(t=>{
          ...
          return (
            <div key={t.id} style={{display:'grid',gridTemplateColumns:COLS,gap:4,alignItems:'center',padding:'3px 2px',
              background:oggi?'rgba(245,158,11,0.07)':isW?'rgba(34,197,94,0.07)':isL?'rgba(239,68,68,0.07)':t.id<=4?'rgba(234,179,8,0.05)':t.id<=8?'rgba(56,189,248,0.05)':'#141414',
              border:`1px solid ${oggi?'rgba(245,158,11,0.7)':isW?'rgba(34,197,94,0.22)':isL?'rgba(239,68,68,0.22)':t.id<=4?'rgba(234,179,8,0.18)':t.id<=8?'rgba(56,189,248,0.15)':'#1a1a1a'}`,
              boxShadow:oggi?'0 0 8px rgba(245,158,11,0.35), inset 0 0 12px rgba(245,158,11,0.05)':'none',
              borderRadius:7}}>
