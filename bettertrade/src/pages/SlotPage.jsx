...
const inputStyle = {
  ...
  borderRadius:5, padding:'5px 7px', color:'#e0d9d0',
  fontSize:12, fontFamily:"'Sora',sans-serif", outline:'none',
}

// ── TileCard — componente separato per evitare re-mount ────────────────────────
function TileCard({ tile, isAdmin, onUpdate }) {
  const isW = tile.result === 1
  const isL = tile.result === 0
  return (
    <div style={{
      background: isW ? 'rgba(34,197,94,0.08)' : isL ? 'rgba(239,68,68,0.08)' : '#141414',
      border: `1px solid ${isW ? 'rgba(34,197,94,0.3)' : isL ? 'rgba(239,68,68,0.3)' : '#1e1e1e'}`,
        ...
        {[
          ...
          <div key={field}>
            ...
            <input
              ...
              value={tile[field]}
              disabled={!isAdmin}
              onChange={e => onUpdate(tile.id, field, e.target.value)}
            />
          </div>
        ))}
        <div>
          ...
          <select
            style={{ ...inputStyle, color:'#c9a84c', fontFamily:"'DM Mono',monospace" }}
            value={tile.pronostico}
            disabled={!isAdmin}
            onChange={e => onUpdate(tile.id, 'pronostico', e.target.value)}
          >
            <option value="">-</option>
          ...
          <div>
            ...
            <div style={{ display:'flex', gap:4 }}>
              {[{ v:null, l:'—' }, { v:1, l:'✓' }, { v:0, l:'✗' }].map(b => (
                <button key={String(b.v)} onClick={() => onUpdate(tile.id, 'result', b.v)}
                  style={{
                    flex:1, padding:'5px 2px', borderRadius:5, fontSize:12, cursor:'pointer',
...
// ── Griglia — componente separato ─────────────────────────────────────────────
function Griglia({ tiles, isAdmin, onUpdate }) {
  return (
    <div>
      {!isAdmin && (
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10 }}>
        {tiles.map(tile => (
          <TileCard key={tile.id} tile={tile} isAdmin={isAdmin} onUpdate={onUpdate} />
        ))}
      </div>
    </div>
  )
...
// ── Schedine — componente separato ────────────────────────────────────────────
function Schedine({ tiles, isAdmin, sched, isWip, onUpdate }) {
              ...
              {c.pos.map(pos => {
                ...
                return (
                  <div key={pos} style={{ display:'flex', alignItems:'center', gap:8, padding:'5px 0', borderBottom:'1px solid #181818' }}>
                    ...
                    <input
                      type="number" step="0.01" min="1" placeholder="quota"
                      value={t.quota}
...
export default function SlotPage({ tiles, setTiles }) {
  ...
  return (
    <div style={{ padding:'16px' }}>
      ...
      {activeTab === 'griglia'
        ? <Griglia  tiles={tiles} isAdmin={isAdmin} onUpdate={updateTile} />
        : <Schedine tiles={tiles} isAdmin={isAdmin} sched={sched} isWip={isWip} onUpdate={updateTile} />
      }
    </div>
  )
}
