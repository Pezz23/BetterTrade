...
const COMBOS = [
  { id:1, nome:'Tris 1-5-2',       tipo:'tris',    pos:[1,5,2]              }, // riga 1 slot
  { id:2, nome:'Tris 1-9-4',       tipo:'tris',    pos:[1,9,4]              }, // diagonale ↘
  { id:3, nome:'Tris 6-9-7',       tipo:'tris',    pos:[6,9,7]              }, // riga 2 slot
  { id:4, nome:'Tris 3-8-4',       tipo:'tris',    pos:[3,8,4]              }, // riga 3 slot
  { id:5, nome:'Tris 3-9-2',       tipo:'tris',    pos:[3,9,2]              }, // diagonale ↙
  { id:6, nome:'Quaterna 1-2-3-4', tipo:'quaterna',pos:[1,2,3,4]            }, // angoli gialli
  { id:7, nome:'Quaterna 5-6-7-8', tipo:'quaterna',pos:[5,6,7,8]            }, // lati blu
...
function SlotVisiva({ tiles }) {
  ...
  function getTileStyle(partita) {
    const t = tiles.find(t => t.id === partita)
    const base = TILE_BASE[partita]
  ...
  // Calcola quali combinazioni sono vinte
  const winCombos = COMBOS.filter(c => comboStatus(tiles, c.pos) === 'win').map(c => c.id)

  return (
    <div style={{ marginTop:20, paddingTop:16, borderTop:'1px solid #1a1a1a' }}>
      <div style={{ fontSize:9, color:'#555', fontFamily:"'DM Mono',monospace", letterSpacing:4, textTransform:'uppercase', marginBottom:12, textAlign:'center' }}>
        Slot
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, maxWidth:280, margin:'0 auto' }}>
        {SLOT_GRID.flat().map(partita => {
          const ts = getTileStyle(partita)
          const t  = tiles.find(t => t.id === partita)
          return (
            <div key={partita} style={{
              background: ts.bg,
              border: `2px solid ${ts.border}`,
              borderRadius: 10,
              padding: '10px 6px',
              textAlign: 'center',
              boxShadow: ts.glow ? `0 0 12px ${ts.glowColor}, 0 0 24px ${ts.glowColor}` : 'none',
              transition: 'all 0.3s ease',
              position: 'relative',
            }}>
              {/* Numero partita */}
              <div style={{ fontSize:11, fontWeight:700, color: ts.color, fontFamily:"'DM Mono',monospace", marginBottom:2 }}>
                {partita}
              </div>
              {/* Pronostico */}
              <div style={{ fontSize:13, fontWeight:700, color: ts.color, fontFamily:"'DM Mono',monospace" }}>
                {t?.pronostico || '-'}
              </div>
              {/* Esito */}
              {hasResults && (
      ...
      {winCombos.length > 0 && (
        <div style={{ marginTop:14, display:'flex', flexWrap:'wrap', gap:6, justifyContent:'center' }}>
          {COMBOS.filter(c => winCombos.includes(c.id)).map(c => (
            <div key={c.id} style={{
              fontSize:10, fontWeight:600, padding:'3px 10px', borderRadius:20,
              background: TIPO_BG[c.tipo], color: TIPO_COLOR[c.tipo],
              border: `1px solid ${TIPO_COLOR[c.tipo]}55`,
              fontFamily:"'DM Mono',monospace",
              boxShadow: `0 0 8px ${TIPO_COLOR[c.tipo]}44`,
            }}>
              ✓ {c.nome}
            </div>
          ))}
