...
//
// Uso:  node scripts/s2-doppia1x2.js [--soglia=0.60] [--quote=bet365|pinnacle]
...
// Quote 1X2 [casa, pareggio, ospite] secondo la fonte.
  ...
  for (const gg of perData) {
    ...
    if (training.length >= MIN_STORICO) {
      ...
      for (const r of gg.rs) {
        ...
        // esito 1X2 col modello più sicuro
        let best = 0;
        for (let i = 1; i < 3; i++) if (probs[i] > probs[best]) best = i;
        if (!quote[best]) continue; // manca la quota
...
// ---- report ----
console.log(`S2 — doppia 1X2 · soglia prob ${(SOGLIA * 100).toFixed(0)}% · quote ${fonte}`);
